...
export const getAllDepartmentsView = async () => {
    try {
        const DEPARTMENTS_URL = `${process.env.NEXT_PUBLIC_BASE_API}/academic/departments-view/`;
        const token = await getValidToken();
        ...
        const rawResult = await res.json();
        return { success: true, data: rawResult.data || rawResult };
    } catch (error) {
        console.error('[Auth] Error:', error);
        return { success: false, message: 'An error occurred' };
    }
